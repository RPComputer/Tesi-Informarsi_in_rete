...
def insert_data(article, af, lista, gs, sub, sent):
	...
	sentiment_query = ("UPDATE articoli SET sentiment = %s, subjectivity = %s, pol_sentiment = %s WHERE link = %s")
	sentiment_data = (gs, sub, sent, article[0])
	try:
...
def article_handler(a):
	...
	insert_data(a, article_frequencies, word_dict, global_sentimet, subjectivity, sentiment_calcolato)
...
def update_dict(words_dict, list):
	for d in list:
		for key, value in d.items():
			if key in words_dict.keys():
				words_dict[key] = words_dict[key] + value
			else:
...
if __name__ == "__main__":
	...
	while True:
		...
		pool = multiprocessing.Pool(processes = 5, initializer = init, initargs = (progress, n_articoli, dict_list))
